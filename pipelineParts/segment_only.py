...
for path in Path().glob('input/*.tif'):
    ...
    #Make sure folders for output exist
    if not os.path.exists(f'output/cropped_{fullname}'):
        os.makedirs(f'output/cropped_{fullname}')
    if not os.path.exists(f'output/segmented_{fullname}'):
        os.makedirs(f'output/segmented_{fullname}')
    ...
    cv2.imwrite(f'output/cropped_{fullname}/{fullname}_{i}.tif',out2)
    ...
    if max_i is not None:
        ...
        cv2.imwrite(f'output/cropped_{fullname}/{fullname}_{i}B.tif',out2)
        ...
        cv2.imwrite(f'output/segmented_{fullname}/glomTuft_{fullname}_{i}.jpg',masked)
